...
.controller('DashboardCtrl', ['$scope', '$http', function ($scope, $http) {
    ...
    if (err) {
      ...
      $scope.chain = response.data.map(function(transaction) {
        ...
        switch(type) {
          ...
          case 'PlaceOrder':        transactionSubmitter = transaction.eventsEmitted[0].orderer.replace('resource:org.acme.vehicle.Person#', '');
                                    displayTransaction = true;
                                    break;

          case 'UpdateOrderStatus': transactionSubmitter = transaction.eventsEmitted[0].order.vehicleDetails.make.replace('resource:org.acme.vehicle.Manufacturer#', '');
                                    displayTransaction = true;
  ...
  function openWebSocket() {
    ...
    websocket.onmessage = function (event) {
      ...
      var caller = message.orderer ? message.orderer.replace('resource:org.acme.vehicle.Person#', '') : message.order.vehicleDetails.make.replace('resource:org.acme.vehicle.Manufacturer#', '');
      var status = message.order ? message.orderStatus : null;
      $scope.addBlock(message.eventId.split('#')[0], message.$class.replace('org.acme.vehicle.', '').replace('Event', ''), caller, status);
      $scope.$apply();
    }
  }
